...
# v2: combine move and grasp
# t3: use the first line of traj15 as the inital position
...
print("jpos")
print(map(float, firstLineRunlevel3[jpos_index:jpos_index+16]))
print("mpos")
print(map(float, firstLineRunlevel3[mpos_index:mpos_index+16]))
print("pos")
print(map(int, firstLineRunlevel3[pos_index:pos_index+6]))
print("ori")
print(map(float, firstLineRunlevel3[dori_index:dori_index+18]))
print("grasp")
print(map(float, firstLineRunlevel3[grasp_index:grasp_index+2]))
...
for i in range(1,10): # [1] initial values
    ...
    newLine[grasp_index:grasp_index+2] = line[grasp_index:grasp_index+2] # filed.grasp_d
    line = reader.next()
...
while True:
    ...
    newLine[dpos_index:dpos_index+3] = ( np.array( map(float, firstLineRunlevel3[dpos_index:dpos_index+3]) )
    ...
    newLine[grasp_index:grasp_index+2] = line[grasp_index:grasp_index+2] # firstLineRunlevel3[grasp_index:grasp_index+2] # filed.grasp_d
    line = reader.next()
...
graspSteps = 2000
graspSpacing = float(0 - 1.5) / graspSteps
print(graspSpacing)
for i in range(1,graspSteps):
    newLine[runlevel_index] = "3"
    last_seq = last_seq + 1
    newLine[packet_index] = str(last_seq)
    newLine[grasp_index:grasp_index+2] = line[grasp_index:grasp_index+2]
    line = reader.next()
    #print([1+(i*graspSpacing), 1+(i*graspSpacing)])
    writer.writerow(newLine)
