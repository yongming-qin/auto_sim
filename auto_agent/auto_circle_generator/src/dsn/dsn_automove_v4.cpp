 ...
 * dsn_automove_v4: 2018/11/19 Use straight line for expolation of the planned path instead of fixed short movement
 *                  !! The result is not good. As the pos is integer. It's not easy to divide the three joints' values
 *                  evenly. (can not use float) 
 * 
 *
*/
...
int main(int argc, char **argv)
{
    ...
    if (true) // for future
    {
        ...
        /* --------------------------------------------------- */
            ...
            inFile1.open("/home/yq/Simulator/mine_ws/homing_to_object.csv");
            if (!inFile1) {
...
#define MOVETOCONTAINER
#ifdef MOVETOCONTAINER            
            // move a little up
            change.keep_tl_reset();
        ...
        /* --------------------------------------------------- */   
            ...
            inFile2.open("/home/yq/Simulator/mine_ws/object_to_container.csv");
            if (!inFile2) {
                cout << "Unable to open file" << endl;
                return 1;
            }
...
//#define CONFIRM
                ...
                )
                {
                    ...
                    if (it->b_tl) {
                        ...
                        VectorXd abs_dif(3), dif(3), incr(3);
                        if (AtStart) {
                            abs_dif(0) = it->tf.translation.x - act_pos_d[0];
                            abs_dif(1) = it->tf.translation.y - act_pos_d[1];
                            abs_dif(2) = it->tf.translation.z - act_pos_d[2];
                            AtStart = false;
                        }
                        int max_dis = abs_dif.lpNorm<Infinity>();
                        int n_each_seg = max_dis / V;
                        VectorXd v_each_seg(3);
                        v_each_seg = abs_dif / 3;
                        ...
                        for (int i = 0; i < 3; ++i)
                        {
                            ...
                            else {incr(i) = (dif[i] > 0) ? v_each_seg(i) : -v_each_seg(i);}
