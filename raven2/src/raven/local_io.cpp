    ...
    for (int i = 0; i < NUM_MECH; i++)
    {
        ...
        for (int j = 0; j < 3; j++)
            for (int k = 0; k < 3; k++) {
                ...
                //std::cout << "local_io: _ori->R[j][k]: " << _ori->R[j][k] << std::endl;
            }
...
bool segLabelCallback(raven_2::SegLabel::Request &req, raven_2::SegLabel::Response &res);
...
ros::ServiceServer seg_label_service;
int seg_label = 0;
...
int init_ravenstate_publishing(ros::NodeHandle &n)
{
    ...
    seg_label_service = n.advertiseService("add_seg_label", segLabelCallback);
...
void autoincrCallback(raven_2::raven_automove msg)
{
    ...
    //std::cout << "in autoincrCallback" << std::endl;
    ...
    {
        armidx = (i == 1) ? 1 : 0;
        //std::cout << "loop armidx: " << armidx << std::endl;
#endif
        // add position increment
        tf::Vector3 tmpvec = in_incr[armidx].getOrigin();

        //std::cout << "data1.xd[armidx].x: " << data1.xd[armidx].x << std::endl;
        //std::cout << "increment: " << tmpvec[0] << " " << tmpvec[1] << " " << tmpvec[2] << std::endl;
        ...
        if (q_temp != tf::Quaternion::getIdentity())
        {
            ...
            for (int j = 0; j < 3; j++)
                for (int k = 0; k < 3; k++) {
                    data1.rd[armidx].R[j][k] = rot_mx_temp[j][k];
                    //std::cout << "block armidx: " << armidx << std::endl;
                    //std::cout << "local_io Callback: rot_mx_temp[j][k]: " << rot_mx_temp[j][k] << std::endl;
                }
        }
        ...
        data1.rd[armidx].grasp -= grasp_gain * msg.del_pos[armidx];
        //std::cout << "data1.rd[armidx].grasp: " << data1.rd[armidx].grasp << std::endl;
        if (data1.rd[armidx].grasp > graspmax)
            data1.rd[armidx].grasp = graspmax;
        else if (data1.rd[armidx].grasp < graspmin)
            data1.rd[armidx].grasp = graspmin;
...
/**
 * \brief The autonomous agend will send the label of the segment for current movement.
 * 
 */
bool segLabelCallback(raven_2::SegLabel::Request &req, raven_2::SegLabel::Response &res)
{
    seg_label = req.label_from_auto_agent;
    res.ret = 1;
    ROS_INFO("request received, label: %d", (long int)req.label_from_auto_agent);
    return true;
}
...
void publish_ravenstate_ros(robot_device *dev, param_pass *currParams)
{
    ...
    for (int i = 0; i < NUM_MECH; i++)
    {
        ...
        for (int m = 0; m < numdof; m++)
        {
            ...
            // for Samin's segment classification
            msg_ravenstate.seg_label = seg_label;
...
void publish_joints(robot_device *device0)
{
    ...
    //std::cout << "joint_states published before if toSec: " << d.toSec() << std::endl;
    ...
    // update joint_state
    //std::cout << "joint_states published after sensor_msgs" << std::endl;
    ...
    int left, right;
    //std::cout << "joint_states published before if" << std::endl;
    ...
    //std::cout << "joint_states published after if" << std::endl;
